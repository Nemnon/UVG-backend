...
import {ZoomReportAnswerUser} from "@/components/zoom/ZoomTypes";
import {IUserPost} from "@/components/http/routes/usersAPI/userPost";
import {IdbSchedule, TeenPresence, ZoomPresenceSortUser} from "@/components/dataBase/DataBaseTypes";
import {PresenceUser, UserPresenceCalculated} from "@/components/userPresence/UserPresenceTypes";
...
export const getUserIdFromKey = async (id: string) => {
    if(!id || id === ''){
        return 0
    }
    let conn;
...
export const getUserWasToday = async (user: IncomingUsersData) => {
    let conn;
    try {
        conn = await pool.getConnection()
...
export const addUserEvent = async (user: IncomingUsersData) => {
    if(user.ping !== 0){
        return
    }
...
export const addUserToTimeLine = async (user: IncomingUsersData, cam: string | boolean ='no') => {
    if(user._userId === 0 || cam === false){
        return
    }

    let conn;
    const table = cam === true?'timeline_cam':'timeline'

    try {
        conn = await pool.getConnection()
        const rows = await conn.query(`SELECT * FROM ${table} WHERE DATE(date_enter) = CURDATE() AND user_id=? ORDER BY date_enter DESC LIMIT 1`, [user._userId])
        if(rows && rows.length===0){
            await conn.query(`INSERT INTO ${table}(user_id, date_enter, date_exit) VALUES(?,NOW(),NOW())`,[user._userId])
        } else {
            const now = new Date().getTime()
            const dte = new Date(rows[0].date_exit).getTime()
            if(now > dte){
                if (now - dte < USER_TIMEOUT){
                    await conn.query(`UPDATE ${table} SET date_exit = NOW() WHERE id=?`, [rows[0].id])
                } else {
                    await conn.query(`INSERT INTO ${table}(user_id, date_enter, date_exit) VALUES(?,NOW(),NOW())`, [user._userId])
                }
            }
...
export const getUsersTimeLine = async (date: Date) => {
    let conn;
...
export const addZoomUsersToTimeLine = async (users: ZoomReportAnswerUser[]) => {
    let conn;
    try {
        conn = await pool.getConnection()
        for (const user of users) {
            ...
            if(userId > 0){
                ...
                const userTL = <ZoomPresenceSortUser[]> await conn.query('SELECT user_id, UNIX_TIMESTAMP(date_enter) as date_enter, UNIX_TIMESTAMP(date_exit) as date_exit FROM timeline WHERE DATE(date_enter) = CURDATE() AND user_id=?',[userId])
                ...
                const result: ZoomPresenceSortUser[] = []
                let curEnd = 0
                let curIdx = -1
...
export const saveUsersPresences = async (users: UserPresenceCalculated[]) => {
    let conn;
    try {
        conn = await pool.getConnection()
        for (const user of users) {
            for (const presenceValue of user.presenceValues) {
                try {
                    await conn.query(`INSERT INTO users_presence(user_id, p_id, p_date, p_val) VALUES(?,?,CURDATE(),?)`,[
                        user.userId,
        ...
        result = await conn.query(`
                ...
                FROM                
                    (
                        ...
                        WHERE
                            users_presence.p_id < 5 AND
                            users_presence.p_date >= DATE_SUB(LAST_DAY(NOW()),INTERVAL DAY(LAST_DAY(NOW()))-1 DAY) AND
                            users_presence.p_date <= LAST_DAY(NOW())
                        GROUP BY users.ten
                    ) presence
...
export const updateUser = async (user: IUserPost) => {
    let conn;
    try {
        conn = await pool.getConnection()
        if (user.id === 0) {
...
export const addLog = async (date: Date, msg: string, cnt: number) => {
    let conn;
    try {
        conn = await pool.getConnection()
...
export const ifSchedule = async () => {
    ...
    try {
        ...
        const dbSchedule = <IdbSchedule[]> await conn.query('select * FROM schedule WHERE day_of_week = ?', [now.getDay()])
...
export const getLogs = async () => {
    ...
    try {
        conn = await pool.getConnection()
        return  await conn.query(`select id, DATE_FORMAT(date , '%Y-%m-%d %T') as date, msg, cnt FROM logs`)
    } finally {
        if (conn) conn.release();
    }
}
