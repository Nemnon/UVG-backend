...
import {UserPresenceCalculated} from "@/components/userPresence/UserPresenceTypes";
...
export const saveUsersData = async (users: IncomingUsersData[]) => {
    try {
        const now = new Date()
...
export const saveTenPresence = async () => {
    try {
        ...
        for (let rowIndex = config.UVGGoogleSheet.usersSheet.startRowIndex; rowIndex < sheet.rowCount; rowIndex++) {
            const userIdCell = sheet.getCell(rowIndex, 0).value
            if(userIdCell){
                const sheetTen = tenPresence?.find((u:any)=> 'T'+u.ten === userIdCell)
                if(sheetTen){
...
export const saveToTimeLine = async (users: UserPresenceCalculated[]) => {
    try {
        const date = new Date()
