...
import {ZoomReportAnswer, ZoomReportAnswerUser} from "./ZoomTypes";
...
const getLastMeetingReportFromZoom = async () => {
    const users: ZoomReportAnswerUser[] = []
    const now = getDate()

    const ans = await axios.get(`https://api.zoom.us/v2/report/meetings/${config.zoom.meetingId}/participants?page_size=300`)
    const data = <ZoomReportAnswer> ans.data

    if(data?.participants?.length > 0){
        for (const user of data.participants) {
            user.join_time = getDateTime(<string> user.join_time)
            user.leave_time = getDateTime(<string> user.leave_time)
